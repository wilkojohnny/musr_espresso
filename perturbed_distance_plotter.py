...
def main():
    ...
    #                     default=None, nargs='?')
    parser.add_argument("pw_prefix", help="pw.x input/output prefix")
    parser.add_argument("--nogle", action="store_true", help="Don't make gle file for plot")
    arguments = parser.parse_args()

    pw_prefix = arguments.pw_prefix
    dat_file_location = os.path.basename(pw_prefix) + '.dist'
    gle_file_location = os.path.basename(pw_prefix) + '.gle'
    ...
    input_filenames = glob.glob(pw_prefix + "*.pwi")
    output_filenames = glob.glob(pw_prefix + "*.pwo")

    # sort them out so we can get the files simultaneously
    ...
    # check the number of pwi and pwo files are equal
    assert len(input_filenames) == len(output_filenames)

    for file_i in range(0, len(input_filenames)):
        # do the files
        pwi_file = input_filenames[file_i]
        pwo_file = output_filenames[file_i]
