...
def QErel(prefix,postfix='.out',set_spg=None,set_spgsetting=None,cif_flag=True,XMufile_flag=False,Esort_flag=True,set_ncluster=None,muon_multipl_prec=0.01,dist_cluster_prec=0.1,verbose='min'):
    ...
    filelist=glob.glob(prefix+'*'+postfix) #find all files with names prefix*postfix (prefix has to include the directory if the files are not in the current folder)
    N_files=len(filelist) #number of files found
    if verbose in ['min','max']:
        print(str(N_files),' files found.')
    for filename in filelist:
        if XMufile_flag:
            utilities.file_strchange(filename,'Mu','XMU',newfile_ext='XMU')#change all occurrences of 'Mu' to 'XMU' so that ASE can read in the QE file (XMU is used to avoid turning XMu into XXMu etc.)
            if verbose in ['max']:
    ...
    results_headers=['Mu_xi','Mu_yi','Mu_zi','Mu_xf','Mu_yf','Mu_zf','E (Ry)','dE (Ry)','dE (K)','NN ion','NN dist (A)','Multipl','Dist cluster','Bond order cluster', 'file_name']
    results_detailedheaders=['Atom','x_i','y_i','z_i','x_f','y_f','z_f','Mu-dist.(A)','Displ.(A)','Cart_x_i','Cart_y_i','Cart_z_i','Cart_x_f','Cart_y_f','Cart_z_f','Change in dist. to final muon site (A)']
    ...
    units2006=units.create_units('2006') #raised the issue of espresso files being read with values for physical constants from 2006 with developers. Might be changed at some point soon.

    #look at each file in turn
    for iterator,filename in enumerate(filelist):
        if verbose in ['min','progbar']:#write a progress bar
            sys.stdout.write("\rFile: "+str(iterator+1).rjust(len(str(N_files)))+'/'+str(N_files)+' {0}'.format(utilities.progbar(iterator+1,N_files)))
        if verbose in ['max']:
        ...
        resultsi_all.append(file_i) #add recently read initial configuration to array holding all initial configurations
    
        #write cif file of final configuration if cif_flag is set
        if cif_flag:
            file_f.write(filename.replace(postfix,'.cif'),format='cif') #this is the ASE write command for ASE atoms objects, NOT the default python write function
            if verbose in ['max']:
        ...
        if iterator==0:
            ...
            results_detailedpandas=np.array(np.zeros((N_files,N_atoms+2,len(results_detailedheaders))),dtype=str) #have two extra rows for: column headers and for summary row
        ...
        final_energy=file_f.get_total_energy()/units2006['Rydberg'] #total energy of final scf calculation in Rydberg
        cartcoords_change=cartcoords_f-cartcoords_i #vector change in Cartesian position of each atom (in Angstrom)
        cartcoords_dist=np.sqrt((cartcoords_change*cartcoords_change).sum(axis=1)) #distance (in Angstrom) from final to initial position of each atom. This appears to be much faster than using linal.norm, see: https://stackoverflow.com/questions/9171158/how-do-you-get-the-magnitude-of-a-vector-in-numpy
        
        results_summary.at[iterator,['file_name']] = filename
        results_detailedpandas[iterator,range(2,N_atoms+2),0]=atoms_symbols #list of chemical symbols for the ions
        results_summary.at[iterator,['Mu_xi','Mu_yi','Mu_zi']]=np.round(fractcoords_i[-1],3)#initial muon position. rounded to 3 decimals to remove floating point glitches
        results_detailedpandas[iterator,range(2,N_atoms+2),1:4]=np.array(np.round(fractcoords_i,3),dtype=str)#save all initial fractional positions
        results_summary.at[iterator,['Mu_xf','Mu_yf','Mu_zf']]=fractcoords_f[-1] #final fractional muon position
        results_detailedpandas[iterator,range(2,N_atoms+2),4:7]=np.array(fractcoords_f,dtype=str)#save all final positions
        Mu_nndist=file_f.get_distances(-1,range(0,N_atoms),mic=True) #calculate all (final) distances (in Angstrom) to the muon (accounting for equivalent atoms in adjacent cells)
        ...
        NN_mindist_index, NN_mindist_value = min(enumerate(Mu_nndist[0:-1]), key=operator.itemgetter(1)) #find the smallest final distance to the muon and the index of that atom
        results_summary.at[iterator,'NN ion']=atoms_symbols[NN_mindist_index] #chemical symbol of muon's nearest neighbour ion
        results_summary.at[iterator,'NN dist (A)']=NN_mindist_value #nearest neighbour ion distance to muon
    
        Mu_nndistchange=copy.deepcopy(file_i) #create an actual copy of the initial configuration. Necessary as otherwise the next line will change file_i!
        ...
        Mu_nndistchange=Mu_nndistchange.get_distances(-1,range(0,N_atoms),mic=True) #distance between all initial atom positions and final muon position
        results_detailedpandas[iterator,range(2,N_atoms+2),15]=np.array(Mu_nndist-Mu_nndistchange,dtype=str) #change between distances to final muon position
    
        #write first two rows for each run details: short summary and then column headers.
        results_detailedpandas[iterator,1,range(len(results_detailedheaders))]=np.array(results_detailedheaders,dtype=str)
        results_detailedpandas[iterator,0,range(len(results_detailedheaders))]=np.array([atoms_symbols[-1],fractcoords_i[-1,0],fractcoords_i[-1,1],fractcoords_i[-1,2],fractcoords_f[-1,0],fractcoords_f[-1,1],fractcoords_f[-1,2],NN_mindist_value,'Final','Energy',final_energy,'Ry',final_energy*units.Ry,'eV',final_energy*units.Ry/units.kB,'K'],dtype=str)
    
    results_summary.at[:,'dE (Ry)']=results_summary.loc[:,'E (Ry)']-np.amin(results_summary.loc[:,'E (Ry)'])#subtract minimum energy from all other energies
    ...
    original_latt=copy.deepcopy(file_i) #load the initial configuration from the last file read in. NEEDS the deepcopy function as otherwise the .pop() function will change the original!
    original_latt.pop(i=-1) #Remove and return atom at index i (default last), which here is the muon. (to do: generalise this)
    ...
    muon_symbols=['X']*N_files #list of ['X','X',...] with number of elements equal to number of DFT+mu relaxation calculations
    muon_finalsites=results_summary.loc[:,['Mu_xf','Mu_yf','Mu_zf']].values #list of fractional coordinates of final relaxed muon sites. as_matrix() is needed for ASE to recognize it as an array.
    muon_finalatoms=Atoms(muon_symbols,muon_finalsites,cell=original_latt.get_cell()) #create Atoms object with the final relaxed muon sites. This will later be added to a lattice corresponding to one muon site in the original spacegroup
    muon_finalatoms.set_scaled_positions(muon_finalsites) #set scaled positions again since ASE for some reason will not use scaled positions automatically
    for iterator in range(0,N_files):
        muon_latt=spacegroup.crystal(symbols=muon_symbols[iterator],basis=muon_finalsites[iterator],spacegroup=spg,setting=spgsetting,cell=original_latt.get_cell(),symprec=muon_multipl_prec)
        ...
        muon_multiplicities[iterator]=muon_latt.get_number_of_atoms()#multiplicity of final muon site, up to symmetry precision set in previous line
        muon_latt.extend(muon_finalatoms)#add all final muon sites to created lattice, in order to calculate the shortest distance between all symmetry equivalent positions of a particular muon site with all others (including itself)
        muon_distances[iterator]=np.amin(muon_latt.get_all_distances(mic=True)[-N_files:,0:np.int_(muon_multiplicities[iterator])],axis=1)
        ...
        results_summary.at[iterator,'Multipl']=muon_multiplicities[iterator] #add multiplicities to the results summary
    if verbose in ['min','max']:
    ...
    for irow in range(0,N_files): #cycle through the final muon sites
        if dist_cluster[irow]==0: #if the currently considered muon site has not been assigned a cluster then ...
            ...
            for icolumn in range(0,N_files): # ... cycle through all the columns (in the given row) and ...
                if muon_distances[irow,icolumn]<=dist_cluster_prec: #... if the muon-muon distance is less than the precision set ...
    ...
    gene_bonds=Gene(name='bond_order_pars',weight=1.0,params={'cutoff_radius':3.0})#bond order parameters, which are projections of the local environment of an atom on spherical harmonics
    phClust = PhylogenCluster(aColl, [gene_bonds])#create the cluster for the given choice of gene(s)
    clust_inds, clust_slices = phClust.get_kmeans_clusters(ncluster)#Returns indices and slices representing the clusters
    results_summary.at[range(0,N_files),'Bond order cluster']=clust_inds #add clustering to the results summary table
    if verbose in ['min','max']:
        print('Bond order parameter clustering done with ',str(ncluster),' clusters.')
        
    #sorting results by final energy
    index_Esorted=[[x,results_summary.loc[x,'E (Ry)']] for x in range(0,N_files)] ##array with index and energy for the configuration with that index
    if Esort_flag:
    ...
    with utilities.open_csv((prefix+'.csv').replace('..csv','.csv'), 'w') as csvfile: #open_csv chooses correct encoding based on whether python 2 or 3 is used
        write_csv=csv.writer(csvfile, delimiter=',',quotechar='|', quoting=csv.QUOTE_MINIMAL) #the str(u'') constructs are needed if unicode_literals are imported. See:https://github.com/jdunck/python-unicodecsv/issues/36
        ...
        write_csv.writerow(['A (Ang)','B (Ang)','C (Ang)','alpha','beta','gamma','cell_volume (Ang^3)','N_atoms','spg H-M'])
        write_csv.writerow(np.concatenate((resultsi_all[0].get_cell_lengths_and_angles(),[resultsi_all[0].get_volume()],[resultsi_all[0].get_number_of_atoms()],[spg])))
